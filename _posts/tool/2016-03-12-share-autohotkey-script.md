...
2. Ctrl+Alt+i -        > 复制文本，并处理，对文本中最后的数值(十进制或十六进制0x)自动+1，然后粘贴；分别对末尾的“:”和";"进行了处理，回车再粘贴，便于C语言的自增复制（之前这种重复性工作很苦恼啊），对于case语句也挺有用，如选中“case 0:”，然后一直按Ctrl+Alt+i，就自动出来“case 1:”,“case 2:”等行。
...
7. Ctrl+Alt+x        -> 复制并自动交换==,<=,>=,=,<,>,!=等符号两边的变量，对于<=等则同时交换为>=等，如"a == 0"，变为"0 == a","intx[0] < 1"变为“1 > intx[0]"，对于C编程中条件语句的变化十分方便(个人习惯，数值放前面，变量放后面，如if(1 == a)这样，经常有要改的情况，so...)。
...
26. Win+a                 -> 打开/激活/最小化 Al*tiu*m De*sign*er程序。
        27.1 Alt+滑轮上        -> 转到上一个标签
        27.2 Alt+滑轮下        -> 转到下一个标签
